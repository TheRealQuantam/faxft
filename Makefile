.PHONY: all clean dir run

GAME_PRE := fax
FTCFG_PRE := fax
BASE_ROM := Faxanadu_(USA)_(Rev_A)_512k.nes
PATCH_ROM_512KB := Faxanadu_(USA)_(Rev_A)_512k.nes
PATCH_ROM := Faxanadu_(USA)_(Rev_A).nes
COMPILE_OPTS := -D ROM_512KB -D MMC3_NO_DEFINE_CALL_UPDATE_SOUND
LINK_OPTS :=
DEMO_CFG_NAME := democfg.json5 

# Requires js65 from https://github.com/jsnesx/js65
CA65 := js65-bun
LD65 := js65-bun

SRCDIR := $(CURDIR)
STEM := $(notdir $(CURDIR))
BHOPDIR := bhop
BUILDDIR := build\$(GAME_PRE)
CFG_NAME := $(STEM).cfg
ROM_NAME := $(GAME_PRE)ft.nes
DBG_NAME := $(GAME_PRE)ft.mlb
IPS_NAME := $(GAME_PRE)ft.ips
IPS_NAME_512KB := $(GAME_PRE)ft512kb.ips
BPS_NAME := $(GAME_PRE)ft.bps

# Assembler files, for building out the banks
ROOT_ASM_FILES := $(wildcard $(SRCDIR)/*.asm)
ROOT_INC_FILES := $(wildcard $(SRCDIR)/*.inc)
BHOP_ASM_FILES := $(BHOPDIR)/bhop.s
O_FILES := \
  $(patsubst $(SRCDIR)/%.asm,$(BUILDDIR)/%.o,$(ROOT_ASM_FILES))  \
  $(patsubst $(BHOPDIR)/%.s,$(BUILDDIR)/%.o,$(BHOP_ASM_FILES))

MAKE_FT_ROM := python ../makeftrom/makeftrom.py
FTCFG_NAME := $(FTCFG_PRE)ft.ftcfg
DEMO_ROM_NAME := $(GAME_PRE)ftdemo.nes
DEMO_DBG_NAME := $(GAME_PRE)ftdemo.mlb
DEMO_IPS_NAME := $(GAME_PRE)ftdemo.ips
DEMO_IPS_NAME_512KB := $(GAME_PRE)ftdemo512kb.ips
DEMO_BPS_NAME := $(GAME_PRE)ftdemo.bps

ifeq ($(DEMO_CFG_NAME),)
DEMO_CFG_NAME := democfg.json5
endif

all: dir $(ROM_NAME) $(IPS_NAME) $(IPS_NAME_512KB) $(BPS_NAME) $(DEMO_ROM_NAME) $(DEMO_IPS_NAME) $(DEMO_IPS_NAME_512KB) $(DEMO_BPS_NAME)

dir:
	-@mkdir -p $(BUILDDIR)

clean:
	-@rm -rf build
	-@rm -f $(ROM_NAME)
	-@rm -f $(DBG_NAME)
	-@rm -f $(IPS_NAME) 
	-@rm -f $(BPS_NAME) 
	-@rm -f $(DEMO_ROM_NAME) 
	-@rm -f $(DEMO_DBG_NAME) 
	-@rm -f $(DEMO_IPS_NAME) 
	-@rm -f $(DEMO_BPS_NAME)

run: dir $(ROM_NAME)
	rusticnes-sdl $(ROM_NAME)

$(IPS_NAME): $(ROM_NAME)
#	Requires Lunar IPS 1.03 from https://fusoya.eludevisibility.org/lips/index.html
	"Lunar IPS.exe" -CreateIPS $@ "$(PATCH_ROM)" $<

$(IPS_NAME_512KB): $(ROM_NAME)
#	Requires Lunar IPS 1.03 from https://fusoya.eludevisibility.org/lips/index.html
	"Lunar IPS.exe" -CreateIPS $@ "$(PATCH_ROM_512KB)" $<

$(DELTA_NAME): $(ROM_NAME)
#	Requires xdelta3 from https://www.romhacking.net/utilities/928/ renamed to xdelta3.exe
	xdelta3 -e -9 -I 0 -f -s "$(PATCH_ROM)" $< $@
	
$(BPS_NAME): $(ROM_NAME)
#	Requires flips from https://www.romhacking.net/utilities/1040/
	flips --create --bps-delta-moremem --exact "$(PATCH_ROM)" $< $@
	
$(ROM_NAME): $(O_FILES)
	$(LD65) --dbgfile $(DBG_NAME) -r $(BASE_ROM) -m $(BUILDDIR)\map.txt -o $@ $(value LINK_OPTS) $^

$(BUILDDIR)/%.o: $(SRCDIR)/%.asm $(BASE_ROM) $(ROOT_INC_FILES)
	@echo .define SRC_ROM "$(BASE_ROM)" > $(BUILDDIR)/build.inc
	$(CA65) -c -I $(BUILDDIR) -I $(BHOPDIR)/bhop -o $@ $(value COMPILE_OPTS) $<

$(BUILDDIR)/%.o: $(BHOPDIR)/%.s
	$(CA65) -c -o $@ $(value BHOP_COMPILE_OPTS) $<

$(DEMO_ROM_NAME): $(ROM_NAME) $(FTCFG_NAME) $(DEMO_CFG_NAME) 
	$(MAKE_FT_ROM) --ftcfg $(FTCFG_NAME) $(DEMO_CFG_NAME) --input-rom $(ROM_NAME) --output-rom $(DEMO_ROM_NAME) --debug
	copy $(DBG_NAME) $(DEMO_DBG_NAME)
	
$(DEMO_IPS_NAME): $(DEMO_ROM_NAME)
#	Requires Lunar IPS 1.03 from https://fusoya.eludevisibility.org/lips/index.html
	"Lunar IPS.exe" -CreateIPS $@ "$(PATCH_ROM)" $<

$(DEMO_IPS_NAME_512KB): $(DEMO_ROM_NAME)
#	Requires Lunar IPS 1.03 from https://fusoya.eludevisibility.org/lips/index.html
	"Lunar IPS.exe" -CreateIPS $@ "$(PATCH_ROM_512KB)" $<

$(DEMO_BPS_NAME): $(DEMO_ROM_NAME)
#	Requires flips from https://www.romhacking.net/utilities/1040/
	flips --create --bps-delta-moremem --exact "$(PATCH_ROM)" $< $@
